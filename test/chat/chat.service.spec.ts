...
describe('ChatService', () => {
  ...
  describe('sendMessage', () => {
    ...
    /**
     * 보상 트랜잭션 테스트: RabbitMQ 발행 실패 시 MongoDB 메시지 삭제
     * - MessagesService를 통해 메시지 생성 성공
     * - QueueService 발행 실패
     * - 보상 트랜잭션으로 메시지 삭제 호출 확인
     */
    it('should delete message when RabbitMQ publish fails (compensation transaction)', async () => {
      const senderId = 'user-1';
      const receiverId = 'user-2';
      const content = 'Test message';
      const timestamp = new Date('2024-01-01T00:00:00Z');
      const queueError = new Error('RabbitMQ connection failed');

      messagesService.createMessage.mockResolvedValue(
        mockMessageDocument as any,
      );
      messagesService.deleteByMessageId = jest
        .fn()
        .mockResolvedValue(undefined);
      queueService.publishMessage.mockRejectedValue(queueError);

      await expect(
        service.sendMessage(senderId, receiverId, content, timestamp),
      ...
      expect(queueService.publishMessage).toHaveBeenCalled();
      expect(messagesService.deleteByMessageId).toHaveBeenCalledWith(
        mockMessageDocument.messageId,
      );
    });
     ...
     * 보상 트랜잭션 실패 처리 테스트
     * - RabbitMQ 발행 실패
     * - 보상 트랜잭션(메시지 삭제)도 실패하는 경우
     * - 원본 에러가 throw되어야 함
     */
    it('should throw original error when compensation transaction fails', async () => {
      const senderId = 'user-1';
      const receiverId = 'user-2';
      const content = 'Test message';
      const timestamp = new Date('2024-01-01T00:00:00Z');
  ...
  describe('markMessageAsDelivered', () => {
    ...
    /**
     * 보상 트랜잭션 테스트: Redis 작업 실패 시 MongoDB 상태 롤백
     * - MessagesService를 통해 메시지를 전달 완료로 표시 성공
     * - MessageInboxCache에서 제거 실패
     * - 보상 트랜잭션으로 메시지를 미전달 상태로 되돌림
     */
    it('should rollback message status when Redis removal fails (compensation transaction)', async () => {
      const userId = 'user-2';
      const messageId = 'msg-123';
      const redisError = new Error('Redis connection failed');

      messagesService.markAsDelivered.mockResolvedValue(undefined);
      messagesService.markAsUndelivered = jest
        .fn()
        .mockResolvedValue(undefined);
      messageInboxCache.removeFromInbox.mockRejectedValue(redisError);

      await expect(
        service.markMessageAsDelivered(userId, messageId),
      ...
      expect(messagesService.markAsUndelivered).toHaveBeenCalledWith(messageId);
    });

    /**
     * 보상 트랜잭션 실패 처리 테스트
     * - Redis 작업 실패
     * - 보상 트랜잭션(상태 롤백)도 실패하는 경우
     * - 원본 에러가 throw되어야 함
     */
    it('should throw original error when compensation rollback fails', async () => {
      const userId = 'user-2';
      const messageId = 'msg-123';
      const redisError = new Error('Redis connection failed');
  ...
  describe('getOfflineMessages', () => {
    ...
    it('should return offline messages from inbox', async () => {
      ...
      messagesService.findByMessageId
        .mockResolvedValueOnce(messages[0] as any)
        .mockResolvedValueOnce(messages[1] as any);
